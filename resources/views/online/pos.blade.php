...
<div class="container py-4">
    ...
    <div class="mb-4">
        ...
        <a href="{{ route('online.index') }}" class="btn btn-sm {{ request('category') ? 'btn-outline-secondary' : 'btn-orange' }}">ทั้งหมด</a>
        ...
        @forelse($products as $product)
            ...
            <div class="col-6 col-md-4 col-lg-3">
                <div class="card h-100 shadow-sm {{ $totalStock <= 0 ? 'card-out-of-stock' : '' }} position-relative">
                    ...
                    @elseif($totalStock <= 5)
                        <span class="badge badge-low-stock position-absolute top-0 start-0 m-2">⚠️ เหลือ {{ $totalStock }}</span>
                    @endif
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="card-text mb-1">
                            ราคา ({{ $unit->unit_name }}): <span class="price">{{ number_format($unit->price, 2) }}</span> บาท
                        </p>
                    @endforeach
                    
                        <!-- แสดงจำนวนสินค้าคงเหลือ -->
                        <p class="card-text mb-1">จำนวนสินค้าในสต็อก: <span class="stock-quantity">{{ $product->stock_quantity }}</span> ชิ้น</p>

                        
                        <form action="{{ route('online.add') }}" method="POST" class="mt-auto add-to-cart-form" data-product-id="{{ $product->id }}">
                            @csrf
